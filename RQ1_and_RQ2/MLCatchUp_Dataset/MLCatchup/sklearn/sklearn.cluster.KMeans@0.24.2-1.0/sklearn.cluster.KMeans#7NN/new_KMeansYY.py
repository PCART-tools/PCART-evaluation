...
def apply_Kmeans(full_dataset, number_clusters = 8):
    ...
    cltr = KMeans(n_clusters=number_clusters, init='random', n_init=1, max_iter=100, tol=0.001, verbose=0, random_state=None, copy_x=True, algorithm='full')

    #return vector_cluster

np.random.seed(9898)  # 新增
train_set = np.random.rand(100, 2)  # 新增
apply_Kmeans(train_set) #新增
